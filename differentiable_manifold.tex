...
\prop{}{
    ...
    \[
        \left.\frac{d(\mathbf{x}_1 \circ \theta)}{dt}\right|_0=\left.\frac{d(\mathbf{x}_1 \circ \gamma)}{dt}\right|_0,
    \]
...
\pf{
    ...
    \[
        \left.\frac{d(\mathbf{x}_2 \circ \theta)}{dt}\right|_0=\left.\frac{d((\mathbf{x}_2 \circ \mathbf{x}_1^{-1})\circ(\mathbf{x}_1 \circ \theta))}{dt}\right|_0=\left.\frac{d(\mathbf{x}_2 \circ \mathbf{x}_1)}{dx}\right|_p\left.\frac{d(\mathbf{x}_1 \circ \theta)}{dt}\right|_0.
    \]
    Similarly, we have
    \[
        \left.\frac{d(\mathbf{x}_2 \circ \gamma)}{dt}\right|_0=\left.\frac{d(\mathbf{x}_2 \circ \mathbf{x}^{-1}_1)}{dx}\right|_p\left.\frac{d(\mathbf{x}_1 \circ \gamma)}{dt}\right|_0.
    \]
    Thus we show that
    \[
        \left.\frac{d(\mathbf{x}_1 \circ \theta)}{dt}\right|_0=\left.\frac{d(\mathbf{x}_1 \circ \gamma)}{dt}\right|_0\implies \left.\frac{d(\mathbf{x}_2 \circ \theta)}{dt}\right|_0=\left.\frac{d(\mathbf{x}_2 \circ \gamma)}{dt}\right|_0.
    \]
}
...
\[
    v_\gamma(f)=\left.\frac{d(f \circ\gamma)}{dt}\right|_0=\left.\frac{d((f \circ \mathbf{x}^{-1})\circ (\mathbf{x}\circ \gamma))}{dt}\right|_0=\sum_{i=1}^n\left.\frac{\partial (f \circ \mathbf{x}^{-1})}{\partial x^i}\right|_{p}\left.\frac{d({x}^i \circ \gamma)}{dt}\right|_0.
\]
}
...
\dfn{Tangent Space}{
    ...
    \begin{enumerate}
        ...
        \item Leibniz rule: $v([f]_p[g]_p) = [f]_pv([g]_p) + v([f]_p)[g]_p$.
    \end{enumerate}
...
\pf{
    \[
        ...
        f\circ\mathbf{x})}{\partial
        x^j}\right|_{\mathbf{x}^{-1}(p)}\left.\frac{d( x^j\circ\gamma)}{dt}\right|_0v_{\eta_i}\\
        &=\sum_{i=1}^m\sum_{j=1}^n\left.\frac{\partial(y^i \circ
        f\circ\mathbf{x})}{\partial
        x^j}\right|_{\mathbf{x}^{-1}(p)}v_\gamma\left( x^j\right)v_{\eta_i}.
        \end{aligned}
...
\dfn{Tangent Mapping}{
    The \textbf{tangent mapping} of $f:M_1\to M_2$ at point $p$ is defined as the mapping
    \[
    \begin{aligned}
...
\prop{}{
    ...
    \[
        J_pf:=\begin{pmatrix}
            J_{i,j}
         \end{pmatrix}_{m\times n},\qquad J_{i,j}=\left.\frac{\partial(y^i \circ f\circ\mathbf{x})}{\partial x^j}\right|_{\mathbf{x}^{-1}(p)}.
        \]
}
\pf{
    ...
    \[
        \begin{aligned}
            v_{f(\varepsilon_k)}&=\sum_{i=1}^m\sum_{j=1}^n\left.\frac{\partial(y^i \circ f\circ\mathbf{x})}{\partial x^j}\right|_{\mathbf{x}^{-1}(p)}v_{\varepsilon_k}\left( x^j\right)v_{\eta_i}\\
            &=\sum_{i=1}^m\sum_{j=1}^n\left.\frac{\partial(y^i \circ f\circ\mathbf{x})}{\partial x^j}\right|_{\mathbf{x}^{-1}(p)}\delta_{jk}v_{\eta_i}\\
            &=\sum_{i=1}^m\left.\frac{\partial(y^i \circ f\circ\mathbf{x})}{\partial x_k}\right|_{\mathbf{x}^{-1}(p)}v_{\eta_i}\\
            &=\begin{pmatrix}
            v_{\eta_1}&v_{\eta_2}&\cdots&v_{\eta_m}
            \end{pmatrix}
            \begin{pmatrix}
            \left.\frac{\partial(y^1 \circ f\circ\mathbf{x})}{\partial x_k}\right|_{\mathbf{x}^{-1}(p)}\\
            \left.\frac{\partial(y^2 \circ f\circ\mathbf{x})}{\partial x_k}\right|_{\mathbf{x}^{-1}(p)}\\
            \vdots\\
            \left.\frac{\partial(y^m \circ f\circ\mathbf{x})}{\partial x_k}\right|_{\mathbf{x}^{-1}(p)}
            \end{pmatrix}.
        \end{aligned}
    \]
    Hence 
