...
%     \end{aligned}
% \]
% be the coordinate function of smooth manifold $M$ at point $p$ with given coordinate chart $(U, \mathbf{x})$. We have
% \[
...
% \]
% Thus, we can write the derivative in component form as
% \[
...
% \]
% We denote by $\mathcal{C}^{\infty}_p(M)$ the space of functions on $M$ that are smooth at the point $p$. It is easy to see that $\tilde{x}_1,\tilde{x}_2,\cdots,\tilde{x}_n$ all belong to $\mathcal{C}^{\infty}_p(M)$. Given a derivative value $\left.\frac{d}{dt}(\mathbf{x}^{-1} \circ \gamma)\right|_0$, we can define the functional
