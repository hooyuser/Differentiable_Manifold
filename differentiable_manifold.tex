...
\prop{Differential as a Cotangent Vector}{
    Let $f\in C^\infty(M)$ and $p\in M$. Then the differential $(\d f)_p$ can be considered as a cotangent vector in $T_p^*M$ by composing it with the natural isomorphism $T_{f(p)}\mathbb{R}\cong \mathbb{R}:\lambda \left.\partial\right|_{f(p)}\mapsto\lambda$
    \[
