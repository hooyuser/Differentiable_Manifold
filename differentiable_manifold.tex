...
% \]
% We denote by $\mathcal{D}_p(M)$ the space of functions on $M$ that are smooth at the point $p$. It is easy to see that $\tilde{x}_1,\tilde{x}_2,\cdots,\tilde{x}_n$ all belong to $\mathcal{D}_p(M)$. Given a derivative value $\left.\frac{d}{dt}(\mathbf{x}^{-1} \circ \gamma)\right|_0$, we can define the functional
